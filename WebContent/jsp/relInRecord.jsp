...
<script type="text/javascript">
	$(function() {
		//记录最大库存量
		var maxnums = 9999999;//该药品当前库存
		var did = $("#drugid").val();//当前的药品id
		var oldnums = $("#oldnums").val();//更改之前的数量
		if(did.length != 0){
		...
		$("form").submit(function(){
			...
			if($("#nums").val().length != 0){
				var nums = $("#nums").val();
				if(parseInt(oldnums)>parseInt(nums)){
					if((parseInt(maxnums)-parseInt(oldnums)+parseInt(nums))<0){
...
<input type="hidden" value="${insertresult }" id="resulthidden">
	...
	<center>
		<div class="panel panel-default" style="width: 45%;">
			...
			<form action="/Drugsmanagement/RelInHouseServlet" method="post">
			<input type="hidden" name="oldnums" value="${oldbean.inum }" id="oldnums">
			<input type="hidden" value="${oldbean.iid }" name="iid">
				<!-- Table -->
				<table class="table table-striped table-hover">
					<tr align="center">
						<td>药品名称</td>
						<td>
						<input type="hidden" class="form-control" name="idid" id="drugid" value="${oldbean.did }" >
					...
					<tr align="center">
						...
						<td><input type="text" class="form-control"
							placeholder="请输入数量" name="inum" id="nums" value="${oldbean.inum }">
							</td>
					</tr>
					<tr align="center">
						<td>入库时间</td>
						<td><input type="date" class="form-control" name="idate" id="outtime" value="${oldbean.idate }">
						</td>
					</tr>
					<tr align="center">
						<td>经手人</td>
